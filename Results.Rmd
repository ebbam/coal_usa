...
allcomp <- read_excel(here("final_df_2022logs2.xlsx"))
...
bea_fips_mods <- read_excel(here("FIPSModificationsVA.xlsx"), skip = 1, col_types = c("text", "text","text","text"))

cclist <- unique(allcomp$fips[which(allcomp$active_mines != 0)])
allcomp_cc <- subset(allcomp, fips %in% cclist)

cc_cluster <- read_excel(here("cc_clusters_251.xlsx"))
...
xmat1 <- read.csv("county_adjacency.txt", sep="\t", stringsAsFactors = FALSE, header = FALSE)
...
writexl::write_xlsx(allcomp_logs, here("final_df_2022logs.xlsx"))
...
df_01 <- read_excel(here("mp_US.xlsx"))
...
writexl::write_xlsx(allcomp_logs, here("final_df_2022logs2.xlsx"))
