...
#allcomp <- read_excel(here("Final Data Products/final_df_2022full.xlsx"))
#allcomp <- read_excel(here("Final Data Products/final_df_2022logs.xlsx"))
#allcomp <- read_excel(here("Final Data Products/final_df_2022_ruccomplete.xlsx"))
allcomp <- read_excel(here("Final Data Products/final_df_2022logs2.xlsx"))
LA_list_missing <-as.list(unique(allcomp$fips[which(is.na(allcomp$diff_uer))]))
# Remove LA counties with missing observations
allcomp <- subset(allcomp, !(fips %in% LA_list_missing))
allcomp$REE_bin <- ifelse(allcomp$REE_inv_scaled_realgdp > 0, 1, 0) # 162 mines_diff != 0
allcomp$REE_bin_top90 <- ifelse(allcomp$REE_inv_scaled_realgdp >= 0.001, 1, 0) # 152 mines_diff != 0 # 438 obs 152 mines_diff != 0 in coal subset
allcomp$REE_bin_top75 <- ifelse(allcomp$REE_inv_scaled_realgdp >= 0.005, 1, 0) # 123 mines_diff != 0
allcomp$REE_bin_top50 <- ifelse(allcomp$REE_inv_scaled_realgdp >= 0.0221, 1, 0) # 71 mines_diff != 0
allcomp$REE_bin_top25 <- ifelse(allcomp$REE_inv_scaled_realgdp >= 0.0982, 1, 0) # 29 mines_diff != 0

allcomp$neg_diffuer <- - allcomp$diff_uer

bea_fips_mods <- read_excel(here("Final Data Products/FIPSModificationsVA.xlsx"), skip = 1, col_types = c("text", "text","text","text"))

cclist <- unique(allcomp$fips[which(allcomp$active_mines != 0)])
allcomp_cc <- subset(allcomp, fips %in% cclist)

cc_cluster <- read_excel(here("Final Data Products/cc_clusters_251.xlsx"))
...
```{r variable labels, include = FALSE}
...
for(l in 1:nrow(stat_tests)){
  ...
  print(lvar)
  pval_int = purtest(eval(parse(text = paste0("pdf$",lvar, sep = ""))), pmax = 4, test = "hadri", exo = "intercept")
...
xmat1 <- read.csv("Final Data Products/county_adjacency.txt", sep="\t", stringsAsFactors = FALSE, header = FALSE)
...
espos <- feols(FE_diffuer, allcomp, se = 'twoway')
...
```{r beeswarm, eval = FALSE, include = FALSE}
# summary(pdmif)
library(beeswarm)
# # print(pdmif$GroupFactors)
# # print(pdmif$GroupLoadings)
# pdmif$GroupLoadings
# pdmif$GroupFactors
# as.data.frame()
# rownames(as.data.frame(pdmif$Coefficients) %>%  View() ) = fps
# 
# beeswarm(c(pdmif$Coefficients[1,],pdmif$Coefficients[2,],pdmif$Coefficients[3,]) ~ c(rep(1,236),rep(2,236), rep(3,236)),
#           col = c(rgb(0.25, 0.63, 1, 0.75), rgb(1, 0.88, 0.6, 0.75), rgb(0.97, 0.43, 0.37, 0.75)),
#          #pwcol = groupmem,
...
writexl::write_xlsx(econ_dflong, here("fulleconomydf.xlsx"))
...
etable(feols(diff_uer ~ lag_diff + lag_diff:l(REE_bin_top90, 1) + lag_diff:l(REE_bin_top90, 2) + lag_diff:l(REE_bin_top90, 3) + diff_log_realgdp_pc | fips + year, allcomp, panel.id = ~fips+year,se = 'twoway'))
etable(feols(diff_uer ~ lag_diff2 + lag_diff2:l(REE_bin_top90, 2) + lag_diff2:l(REE_bin_top90, 3) + lag_diff2:l(REE_bin_top90, 4)+ diff_log_realgdp_pc | fips + year, allcomp, panel.id = ~fips+year,se = 'twoway'))
```
...
writexl::write_xlsx(allmodels, here("coefplotdf.xlsx"))
allmodelssave <- read_excel(here("coefplotdf.xlsx"))
...
# NEED diff_log_realgdp_pc
allcomp$TAA_scaled_realgdp <- allcomp$total_taa/allcomp$realgdp
allcomp$realgdp_pc <- allcomp$realgdp/allcomp$pop
allcomp$logrealgdp_pc <- log(allcomp$realgdp_pc)
allcomp$log_realgdp <- log(allcomp$realgdp)
allcomp$lag_closure[which(is.na(allcomp$lag_closure))] <- 0

allcomp$neg_diff <- ifelse(allcomp$mines_diff < 0, 1, 0)
allcomp$pos_diff <- ifelse(allcomp$mines_diff >= 0, 1, 0)
allcomp$log_employed <- log(allcomp$employed)

writexl::write_xlsx(allcomp_logs, here("Final Data Products/final_df_2022logs.xlsx"))

gdp <- read_excel("/Users/ebbamark/Library/CloudStorage/OneDrive-Nexus365/Ebba Dissertation Materials/FINAL_REG_DATASETS/gdp_2001_2019.xlsx")
gdp_01 <- subset(gdp, year == 2001)
gdp_01 <- select(gdp_01, c(fips, realgdp, currentgdp))

df_01 <- read_excel(here("Final Data Products/mp_US.xlsx"))
df_01 <- subset(df_01, year == 2001)
df_01 <- select(df_01, c(fips, year, employed, labour_force, pop, unemployed))
df_gdp_01 <- merge(df_01, gdp_01, by = "fips", all.x = TRUE)

# Broomfield County, Colorado was only created in 2001 so no observation 
# exists for 2001 - cannot compute change. Will retain as NA.
df_gdp_01$fips[which(is.na(df_gdp_01$pop))]


test <- bind_rows(allcomp, df_gdp_01)
test <- test %>% arrange(fips, year)
# test$log_employed <- log(test$employed)
# test$log_lf <- log(test$labour_force)
# test$log_pop <- log(test$pop)
# test$log_unemployed <- log(test$unemployed)
test$realgdp_pc <- test$realgdp/test$pop
test$log_realgdp_pc <- log(test$realgdp_pc)
test$log_realgdp <- log(test$realgdp)
# all.equal(test$log_realgdp1, test$log_realgdp)
sum(is.na(test$log_realgdp_pc))

# Diff pop
# Diff real gdp (current)
# Diff log real gdp  (current)
# Diff real gdp pc  (current)
# MOST IMPORTANT: Diff log real gdp pc  (current)

test1 <- test %>% 
   arrange(fips, year) %>%
   group_by(fips) %>% mutate(diff_pop = c(NA, diff(pop)), 
                             diff_realgdp = c(NA, diff(realgdp)), 
                             diff_log_realgdp = c(NA, diff(log_realgdp)),
                             diff_realgdp_pc = c(NA, diff(realgdp_pc)),
                             diff_log_realgdp_pc = c(NA, diff(log_realgdp_pc)))%>%
   ungroup

test2 <- dplyr::select(test1, -c('realgdp_pc1', 'log_realgdp1'))
# test1 <- test %>% 
#   arrange(fips, year) %>%
#   group_by(fips) %>% mutate(diff_log_employed = c(NA, diff(log_employed)), 
#                             diff_log_unemployed = c(NA, diff(log_unemployed)), 
#                             diff_log_lf = c(NA, diff(log_lf)),
#                             diff_log_pop = c(NA, diff(log_pop)))%>%
#   ungroup

# test1 %>% select(fips, year, employed, log_employed, diff_log_employed, unemployed, log_unemployed, diff_log_unemployed,labour_force, log_lf, diff_log_lf, pop, log_pop, diff_log_pop) %>% View()

allcomp_logs <- subset(test2, year != 2001)
identical(allcomp_logs[,1:(length(allcomp_logs)-5)], allcomp)

writexl::write_xlsx(allcomp_logs, here("Final Data Products/final_df_2022logs2.xlsx"))

# df_VA2 <- df_va(df_VA2)
# df_VA2 = allcomp
# identical(df_VA2, df_VA)
# fips_pre <- as.list(unique(allcomp$fips))
# for(a in 1:length(fips_pre)){
#   f = fips_pre[a]
#   if(f %in% getfips){
#       newfips = names(getfips[which(getfips == f)])
...
#         for(b in 1:length(newfips)){
#           print(newfips[b])
#           copy <- subset(df_VA, fips == f)
#           copy$fips <- newfips[b]
#           df_VA <- rbind(df_VA, copy)
#         }
#       }else{
#       }
#       df_VA <- subset(df_VA, fips != f)
#       print(nrow(df_VA)/18)
#   }
# }
# identical(df_VA2, df_VA)
# identical(select(subset(allcomp, fips == "51901"), -fips), select(subset(df_VA2, fips == "51003"), -fips))
