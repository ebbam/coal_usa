...
```{r, stationarity tests, eval = FALSE}
...
etable(model_7_neg, model_8_pos, model_9_both, signif.code = c(`***` = 0.001, `**` = 0.01, `*` = 0.05, . = 0.1), tex = TRUE)
etable(model_9_both)
...
### Aggregate employment changes
```{r}

neg_coef <- model_9_both$coefficients[2] %>% unname

pos_coef <- model_9_both$coefficients[5] %>% unname

allcomp %>% 
  mutate(agg_diff = -0.00056*mines_diff*unemployed,
         asymm_diff = ifelse(sign(mines_diff) == -1, (neg_diff/100)*mines_diff*unemployed, (pos_diff/100)*mines_diff*unemployed)) %>% 
  group_by(year) %>% 
  summarise(agg_diff = sum(agg_diff, na.rm = TRUE),
            asymm_diff = sum(asymm_diff, na.rm = TRUE))


allcomp %>% 
  mutate(agg_diff = -0.00056*mines_diff*employed,
         asymm_diff = case_when(sign(mines_diff) == -1 ~ (neg_coef*mines_diff*labour_force)/100,
                                sign(mines_diff) == 1 ~ (pos_coef*mines_diff*labour_force)/100,
                                TRUE ~ 0)) %>% #select(fips, year, labour_force, mines_diff, agg_diff, asymm_diff) %>%
  group_by(year) %>% 
  summarise(mines_diff = sum(mines_diff, na.rm = TRUE),
            agg_diff = sum(agg_diff, na.rm = TRUE),
            asymm_diff = sum(asymm_diff, na.rm = TRUE)) %>% arrange(agg_diff) %>% 
  
 ungroup %>% 
  summarise(net_diff = sum(agg_diff),
           gross_diff = sum(agg_diff[agg_diff < 0]))
...
rural_2neg <- feols(neg_diffuer ~ l(mines_diff, -1:0) + lag_diff + lag_diff2 + lag_diff3 + diff_log_realgdp_pc | 
               fips + year, cc_ruraldf, panel.id = ~fips+year, se = 'twoway')
medium_2neg <- feols(neg_diffuer ~ l(mines_diff, -1:0) + lag_diff + lag_diff2 + lag_diff3 + diff_log_realgdp_pc | 
               fips + year, cc_mediumdf, panel.id = ~fips+year, se = 'twoway')
urban_2neg <- feols(neg_diffuer ~ l(mines_diff, -1:0) + lag_diff + lag_diff2 + lag_diff3 +diff_log_realgdp_pc | 
...
removed <- subset(allcomp_cc_types, fips %in% const_fips)

# testagain includes only 15 counties
testagain <- filter(allcomp_cc_types, fips %in% const_fips) %>% 
  group_by(fips) %>% 
  summarize(mean = mean(active_mines), max = max(active_mines), min = min(active_mines))

means <- allcomp_cc_types %>% 
  group_by(fips) %>% 
  summarize(mean = mean(active_mines), max = max(active_mines), min = min(active_mines))

removed %>% select(fips, active_mines, mines_diff, lag_diff, lag_diff2) %>% View()
table(means$min)
...
# test2 <- feols(diff_uer ~ mines_diff + lag_diff + lag_diff2 +
#         diff_log_realgdp_pc | fips + year + type, allcomp_cc_types, se = "twoway")
# 
# times <- c("mines_diff", "lag_diff", "lag_diff2")
# glhts <- data.frame()
# for(type in 1:3){
#   for(t in times){
#     v <- glht(test2, linfct = paste0(t,"+`mines_diff:type == ",type,"` = 0")) %>% 
#       tidy %>% pull(estimate) %>% c(type, t, .)
#     glhts <- rbind(glhts, v)
#   }
# }
...
```{r, running Fixest regressions with county type PDMIF, eval = FALSE}
...
ggplot(econ_dflong, aes(x = time, y = model, fill = fill))+
  scale_y_discrete(limits = rev(c("UER", "EMP", "UNEMP", "LF", "POP")), labels = rev(c("Model 1: \u0394 UER","Model 2: \u0394 (log) Employed persons", "Model 3: \u0394 (log) Unemployed persons", "Model 4: \u0394 (log) Labor force", "Model 5: \u0394 (log) Population")))+
  scale_x_discrete(limits = c("t", "t-1", "t-2"), labels = c("0","+1", "+2"))+
    geom_tile(color = "white",
            lwd = 1.5,
            linetype = 1)+
  scale_fill_gradientn(colours=c("navy","royalblue","lightskyblue","white", "mistyrose","red", "darkred"),limits = c(-4,4), na.value = "grey98", labels = c("(-)***","(-)**", "(+/-)*", "(+)**","(+)***"),labs(colour="Sign and Significance Level", size = 0.2))+
  ylab("Outcome Variable")+ 
  xlab("# years following mine closure")+
  theme(panel.background = element_blank(), panel.border = element_blank(), legend.title=element_text(size=14), axis.text.y = element_text(size = 12), text = element_text(size = 16, family = "Bookman"))
#ggsave("economy_model.jpg")
...
ggplot(econ_dflong, aes(x = time, y = model, fill = fill))+
  ...
  ylab("Outcome Variable")+ 
  xlab("# years following mine closure") +
  theme(panel.background = element_blank(), panel.border = element_blank(), legend.title=element_text(size=12), )
...
# 
# etable(ree_model_main, ree_model_test)
...
ree_model_2 <- feols(diff_uer ~ mines_diff + lag_diff + lag_diff2 + lag_diff2:REE_bin_top90 +
                             lag_diff2:l(REE_bin_top90,1) + lag_diff2:l(REE_bin_top90,2) + REE_bin_top90 + diff_log_realgdp_pc | fips + year, allcomp, panel.id = ~fips+year,  se = 'twoway')

glht(ree_model_main, linfct = "mines_diff + lag_diff + lag_diff2 = 0") %>% summary
glht(ree_model_main, linfct = "mines_diff + lag_diff + lag_diff2 + mines_diff:REE_bin_top90 + lag_diff:l(REE_bin_top90,1) + lag_diff2:l(REE_bin_top90,2) = 0") %>% summary

glht(ree_model_main, linfct = "mines_diff + mines_diff:REE_bin_top90 = 0") %>% summary
glht(ree_model_main, linfct = "lag_diff2 + lag_diff2:l(REE_bin_top90,2) = 0") %>% summary

glht(ree_model_2, linfct = "mines_diff + lag_diff + lag_diff2 = 0") %>% summary
glht(ree_model_2, linfct = "mines_diff + lag_diff + lag_diff2 + lag_diff2:REE_bin_top90 + lag_diff2:l(REE_bin_top90,1) + lag_diff2:l(REE_bin_top90,2) = 0") %>% summary

glht(ree_model_2, linfct = "mines_diff + lag_diff + lag_diff2 + lag_diff2:REE_bin_top90 + lag_diff2:l(REE_bin_top90,1) + lag_diff2:l(REE_bin_top90, 2) = 0") %>% summary
glht(main, linfct = "mines_diff + lag_diff + lag_diff2 = 0") %>% summary
glht(ree_model_2, linfct = "lag_diff2 + lag_diff2:l(REE_bin_top90,2) = 0") %>% summary
```
...
# test_sp_err_lag_mines <- lmSLX(fmdiff_uer, data = allcomp_full, index = NULL,  listw = fmatlw, 
#                          lag = TRUE, na.action = na.fail, spatial.error = "b",
#                          model = "within", effect = "twoways", quiet = FALSE)
...
implag %>% plot(choice = "direct", trace = FALSE) 
title(main = "Main title")

for(y in 2002:2019){
  test <- allcomp_full %>% filter(year == y) %>% pull(diff_uer) %>% moran.plot(., listw = fmatlw, return_df = TRUE, plot = FALSE)
test2 <- tibble(labels = test$labels, x = test$x, wx = test$wx)
cc_test <- test2 %>% filter(labels %in% allcomp_cc$fips)
plot(x = cc_test$x, y = cc_test$wx)
  xwx.lm <- lm(test2$wx ~ test2$x)
    abline(xwx.lm)
    abline(h = mean(test2$wx), lty = 2)
    abline(v = mean(test2$x), lty = 2)
    title(main = as.character(y))
}
...
short_spat <- spat_dep %>% filter(year != 2009 & year != 2010) %>% mutate(year = as.factor(year)) %>% group_by(year) %>% mutate(label = ifelse(dist == max(dist), as.character(year), NA)) %>% 
ggplot(aes(x = dist, y = gamma, color = year, group = year)) +
  geom_line() +
  geom_label_repel(aes(label = label),
                   nudge_x = 2,
...
#library(GridExtra)
spat_grid <- grid.arrange(full_spat, short_spat, ncol=2, top = "Variogram of county-level UER: Dissimilarity of counties as a function of distance between them")
#ggsave(spat_grid, filename = "spat_grid.jpg", width = 12, height = 8)

spat_dep %>% group_by(year) %>% summarise(mean_gamma = mean(gamma)) %>% 
  ggplot(aes(x = year, y = mean_gamma)) + geom_line()
...
allcomp_full %>%
  group_by(fips) %>% 
  summarise(ree_inv = (sum(REE_bin_top90) != 0)) %>% 
  merge(., cc_cluster, by = "fips", all.x = TRUE) %>% 
  select(!cluster) %>% 
  mutate(ree_coal_overlap = case_when(
         ree_inv == FALSE & !is.na(type) ~ "No investments, yes coal",
         ree_inv == FALSE & is.na(type) ~ "No investments, no coal",
         ree_inv == TRUE & !is.na(type) ~ "Overlap exists",
         ree_inv == TRUE & is.na(type) ~ "Investment, no overlap")) %>% 
  plot_usmap(data = ., values = "ree_coal_overlap", regions = "counties", col = "gray90", size = 0.04, exclude = c("AK", "HI" )) + 
  scale_fill_viridis_d(name = "County Type", na.value = "gray80") +
...
summary(change1_KSStw) %>% plot# %>% ggsave(filename = "factors")
summary(change2_KSStw) %>% plot #%>% save(filename = "factors.jpg", width = 12, height = 8)

# unique(allcomp_full$fips) %>% 
#   for(f in .){
#     fi <- match(f,unique(allcomp_full$fips))
#     if(!(filter(allcomp_full, fips == f) %>% pull(diff_uer) %>% identical(dep_diff_uer[,fi]))){print(paste(f, "wrong!"))}
#   }

ksstw1 <- change1_KSStw$unob.fact.stru %>% data.frame
names(ksstw1) <- unique(allcomp_full$fips)
ksstw1 <- ksstw1 %>% mutate(year = as.integer(row_number() + 2002)) %>% relocate(year) %>% 
  pivot_longer(!year, names_to = "fips", values_to = "value")

ksstw1 <- df_va(ksstw1)

ksstw2 <- change2_KSStw$unob.factors[,2] %*% t(t(change2_KSStw$ind.loadings)[2,])%>% data.frame
names(ksstw2) <- unique(allcomp_full$fips)
ksstw2 <- ksstw2 %>% mutate(year = as.integer(row_number() + 2002)) %>% relocate(year) %>% 
  pivot_longer(!year, names_to = "fips", values_to = "value")

ksstw2 <- df_va(ksstw2)

ksstw2_both <- change2_KSStw$unob.fact.stru %>% data.frame
names(ksstw2_both) <- unique(allcomp_full$fips)
ksstw2_both <- ksstw2_both %>% mutate(year = as.integer(row_number() + 2002)) %>% relocate(year) %>% 
  pivot_longer(!year, names_to = "fips", values_to = "value")

ksstw2_both <- df_va(ksstw2_both)
#   
# tw1plot <- plot_usmap(data = ksstw1, values = "value", regions = "counties", col = "black", size = 0.07, exclude = c("AK", "HI" )) +
#   scale_fill_viridis(name = "Factor loading",  na.value = "seashell2", option = "magma") +
#   theme(panel.background = element_rect(colour = "black", fill = "gray90"))  +
#   labs(title = "Geographical Distribution of Factor Loadings in {as.integer(frame_time)}") +
#   transition_time(as.integer(year))
# 
#   anim <- animate(tw1plot, fps=10, renderer = gifski_renderer(), device = 'ragg_png', nframes = 17)
#   anim_save("factor_gif_ksstw1.gif", tw1plot)
# 
# tw2plot <- plot_usmap(data = ksstw2, values = "value", regions = "counties", col = "black", size = 0.07, exclude = c("AK", "HI" )) +
#   scale_fill_viridis(name = "Factor loading",  na.value = "seashell2", option = "magma") +
#   theme(panel.background = element_rect(colour = "black", fill = "gray90"))  +
#   labs(title = "Geographical Distribution of Factor Loadings in {as.integer(frame_time)}") +
#   transition_time(as.integer(year))
# 
#   anim <- animate(tw2plot, fps=10, renderer = gifski_renderer(),  device = 'ragg_png')
#   anim_save("factor_gif_tw2plot.gif", anim)
# 
# tw2bothplot <- plot_usmap(data = ksstw2_both, values = "value", regions = "counties", col = "black", size = 0.07, exclude = c("AK", "HI" )) +
#   scale_fill_viridis(name = "Factor loading",  na.value = "seashell2", option = "magma") +
#   theme(panel.background = element_rect(colour = "black", fill = "gray90"))  +
#   labs(title = "Geographical Distribution of Factor Loadings in {as.integer(frame_time)}") +
#   transition_time(as.integer(year))
# 
#   anim <- animate(tw2bothplot, fps=10, renderer = gifski_renderer(), device = 'ragg_png')
#   anim_save("factor_gif_tw2bothplot.gif", anim)
...
dwplot(vline = geom_vline(
  ...
  scale_color_manual(values = c(
                     ...
                     labels = c(
                      ...
                      #`SLM Indirect` = expression(bold("Model 1"^'SLM')*": Indirect Impact"),
                      `SARAR Direct` = expression(bold("Model 1"^'SARAR')*": Direct Impact"),
                      #`SARAR Indirect` = expression(bold("Model 1"^'SARAR')*": Indirect Impact"),
  ...
  scale_y_discrete(
    "", limits = rev,
    labels = c(
      mines_diff = expression("Δ Active Mines"[~t]),
            lag_diff = expression("Δ Active Mines"[~t-1]),
            lag_diff2 = expression("Δ Active Mines"[~t-2]))) +
  ...
  coord_flip() #+
  #scale_y_discrete(limits = rev(levels(labels)))

ggsave("output/coefplot_w_slx.png", width = 10, height = 12)
```
...
# $6,311,624,116 in overall US since 2002
# $122,238,781 in CC counties since 2002 (1.94%)

#counties with REE investments
REE_counties <- 
  filter(allcomp_full, REE_inv != 0) %>% 
  pull(fips) %>%  
  unique()

wtags <- allcomp_full %>% 
  mutate(tag = case_when(fips %in% allcomp_cc$fips ~ 1,
         TRUE ~ 2)) %>% 
  group_by(year) %>% 
  mutate(mean_pop = mean(pop)) %>% 
  select(fips, year, mean_pop, tag)

natlsums <- wtags %>%
  filter(tag == 1) %>% 
  group_by(year) %>%
  summarize(REE = sum(REE_inv), tot_pop = sum(pop)) %>% 
  mutate(ree_pc = REE/tot_pop)
...
# Average population of contiguous US: 305,482,156
allcomp_full %>% 
  group_by(year) %>% 
  summarize(ann_ree = sum(REE_inv), ann_pop = sum(pop)) %>% 
  summarize(ree_tot = sum(ann_ree), pop_tot = mean(ann_pop)) %>% 
  mutate(ree_pc = ree_tot/pop_tot)

# Average population of coal counties: 16,281,589
allcomp_full %>% 
  filter(fips %in% allcomp_cc$fips) %>% 
  group_by(year) %>% 
  summarize(ann_ree = sum(REE_inv), ann_pop = sum(pop)) %>% 
  summarize(ree_tot = sum(ann_ree), pop_tot = mean(ann_pop)) %>% 
  mutate(ree_pc = ree_tot/pop_tot)

# Average population of counties with REE investment (not coal): 246,734,253
allcomp_full %>% 
  filter((fips %in% REE_counties) & (!(fips %in% allcomp_cc$fips))) %>% 
  group_by(year) %>% 
  summarize(ann_ree = sum(REE_inv), ann_pop = sum(pop)) %>% 
  summarize(ree_tot = sum(ann_ree), pop_tot = mean(ann_pop)) %>% 
  mutate(ree_pc = ree_tot/pop_tot)
...
# Exploring coal mine data

```{r}

allcomp_cc %>% 
  group_by(fips) %>% 
  filter(any(total_mines) > 10) %>% 
  ungroup %>% print
  select(fips, year, active_mines,closed_mines,planned_mines) %>% 
  pivot_longer(cols = !c(fips, year), names_to = "class", values_to = "no") %>% 
  ggplot(aes(x = year, y = no, fill = class)) + geom_area() + facet_wrap(~fips, scales = "free")
```
